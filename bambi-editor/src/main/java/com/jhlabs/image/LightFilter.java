...
import org.zimowski.bambi.jhlabs.image.GaussianFilter;
import org.zimowski.bambi.jhlabs.image.ImageMath;
import org.zimowski.bambi.jhlabs.math.Function2D;
import org.zimowski.bambi.jhlabs.math.ImageFunction2D;
import org.zimowski.bambi.jhlabs.vecmath.Color4f;
import org.zimowski.bambi.jhlabs.vecmath.Vector3f;
...
public class LightFilter extends WholeImageFilter {
	...
	protected int[] filterPixels(
		...
		if (bumpSource == BUMPS_FROM_IMAGE
			...
			if (bumpSoftness != 0) {
				...
				/*
				 * for (int i = 0; i < 3; i++ ) { BoxBlurFilter.blur(
				 * bumpPixels, tmpPixels, bumpWidth, bumpHeight,
				 * (int)bumpSoftness ); BoxBlurFilter.blur( tmpPixels,
				 * softPixels, bumpHeight, bumpWidth, (int)bumpSoftness ); }
				 */
		...
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				...
				if (bumpSource != BUMPS_FROM_BEVEL) {
					...
					// Calculate four normals using the gradients in +/- X/Y
					// directions
					int count = 0;
				...
				if (normal.z >= 0) {
					...
					if (reflectivity != 0 && environmentMap != null) {
						...
						setFromRGB(
								envColor,
								getEnvironmentMap(tmpv, inPixels, width, height));// FIXME-interpolate()
				...
				if(index%10000==0) filterProgress(Math.round(((float)index/totalPixels)*100));
			}
		...
		for (int i = 0; i < lightsArray.length; i++) {
			...
			if (nDotL >= 0.0) {
				...
				if (rDotV < 0.0)
					...
					rv = rDotV
							/ (material.highlight - material.highlight * rDotV + rDotV); // Fast
																							// approximation
																							// to
																							// pow
	...
	private int getEnvironmentMap(Vector3f normal, int[] inPixels, int width,
		...
		if (environmentMap != null) {
			...
			return ImageMath.bilinearInterpolate(xWeight, yWeight,
					envPixels[i], envPixels[i + dx], envPixels[i + dy],
					envPixels[i + dx + dy]);
		}
		return 0;
	}
	...
	public ImageFilterOps getMetaData() {
		return ImageFilterOps.Cartoonize;
	}
}